...
export async function remove(id){const rows=await read(),i=rows.findIndex(x=>x.id===id);if(i<0)return null;const [item]=rows.splice(i,1);await fs.writeFile(file,JSON.stringify(rows,null,2));return item}
